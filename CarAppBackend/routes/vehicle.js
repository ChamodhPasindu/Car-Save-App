...
connection.connect(function (err) {
    if (err) {
        ...
        var vehicleTable = "CREATE TABLE IF NOT EXISTS vehicle(vehicle_no VARCHAR (50),user_id INT,brand VARCHAR (50),Model VARCHAR (20),fuel_Type VARCHAR (10),mileage INT,transmission VARCHAR (10),description VARCHAR(255),location VARCHAR (50),date DATE ,CONSTRAINT PRIMARY KEY (vehicle_no))"
...
router.post("/save", upload.array('file', 5), function (req, res, next) {
    ...
    connection.query(vehicleTable, [registrationNo, userId, brand, model, fuelType, mileage, transmission, description,location,date], (err) => {
        if (err) {
            console.log(err)
...
router.put("/",(req,res)=>{
    ...
    connection.query(updateVehicle, [brand,model,fuelType,transmission,mileage,description,registration_no], (err) => {
        if (err){
            console.log(err)
